$(document).ready(function ($) {
    ...
    function AtualizaGridLoad() {
        ...
        var tr;
        $.getJSON('/ViaVarejo/BuscarSituacaoCargas/',
           {
               Nome: $('#Nome').val(),
               sku: $('#Sku').val(),
          ...
          function (json) {
              ...
              $.each(json, function (i, EmpData) {
                  ...
                  tr.append("<th>" + EmpData.skuSellerId + "</th>");
                  tr.append("<th>" + EmpData.description + "</th>");
                  tr.append("<th>" + EmpData.brand + "</th>");
                  tr.append("<th>" + EmpData.skuId + "</th>");
        ...
        $.getJSON('/ViaVarejo/Buscar/',
          ...
          function (json) {
              ...
              $.each(json, function (i, EmpData) {
                  ...
                  tr.append("<th>" + EmpData.skuSellerId + "</th>");
                  tr.append("<th>" + EmpData.title + "</th>");
                  if (EmpData.skuId == 'enabled') {
                      tr.append("<th style='align-content:center'>Ativo</th>");
                  } else {
                  ...
                  tr.append("<th width='40px'><input type='text' id='Estoque' class='form-control' value='" + EmpData.brand + "' name='Estoque-" + EmpData.skuSellerId + "' maxlength='70'></th>");

                  tr.append("<th width='100px'><input type='text' id='Preco' class='form-control' value='" + parseFloat(EmpData.description).toFixed(2) + "' name='Preco-" + EmpData.skuSellerId + "' maxlength='70'></th>");
                  tr.append("<th><a href=\"#\" onClick=\"MyWindow = window.open('/ViaVarejo/Edit/" + EmpData.skuSellerId + "', 'MyWindow', 'width=700,height=600,scrollbars=yes,left=500,top=100'); return false;\">Detalhes Pedido</a></th>");
                  tr.append('<input type="hidden" id="item-to-delete"/>');
                  $('#TblEmpDet').append(tr);
                  $('#TotalPaginas').text(EmpData.TotalBusca);
    ...
    $('#btnPesquisar').click(function () {
     ...
     $('#btnPesquisarLoad').click(function () {
